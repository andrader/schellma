...
from schellma import pydantic_to_llm, json_schema_to_llm
import json
...
class ProductModel(BaseModel):
    ...
    # Array constraints
    categories: list[str] = Field(min_length=1, max_length=5, description="Product categories")
...
class UserOrAdmin(BaseModel):
    entity: Union[User, Admin] = Field(discriminator='type')
...
class APIRequest(BaseModel):
    ...
    url: str = Field(
        examples=["https://api.example.com/users", "https://api.example.com/products/123"],
...
def demonstrate_feature(title: str, model_or_schema, description: str):
...
if __name__ == "__main__":
    ...
    print("   Perfect for generating schemas that LLMs can easily understand and follow.")
